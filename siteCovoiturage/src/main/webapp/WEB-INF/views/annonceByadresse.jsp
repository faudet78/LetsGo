		...
		>
				<h4 align="center">
					<span class="glyphicon glyphicon-search"></span> <strong
						class="text-uppercase" style="color: white;"
					>Rechercher un covoiturage</strong>
				</h4>
		</div>
...
s/chercherAnnonce "></spring:url>'
				class="form-inline"
			>
					<div class="form-group has-feedback field"><input type="text"
						class="form-control" placeholder="Adresse de depart"
						value="${adresseD }" name="adresseD"
					/> <i class="form-control-feedback "><img width="20"
						...
						></i></div> <span style="margin-left: 5px; margin-right: 5px;"
					class="glyphicon glyphicon-arrow-right"
				></span>
					<div class="form-group has-feedback field"><input type="text"
						class="form-control" placeholder="Adresse d'arrivee"
						value="${adresseA }" name="adresseA"
					/> <i class="form-control-feedback "><img width="20"
							style="margin-top: 7px;" height="20" alt="search"
		...
		<div class="col-sm-12 " style="margin-top: 20px;">
			<div align="center"><c:if test="${maxPages>'0' }">
						...
						<ul class="pagination">
							<li><c:if test="${current=='0' }">
									...
									<a class="btn btn-default" aria-label="Previous"
										href='<spring:url value="/annonces/chercherAnnonce?adresseD=${adresseD }&adresseA=${adresseA }&page=${current - 1 }"></spring:url>'
										...
										<c:choose>
											...
											<c:otherwise>
												<a class="btn btn-primary"
													href='<spring:url value="/annonces/chercherAnnonce?adresseD=${adresseD }&adresseA=${adresseA }&page=${p}"></spring:url>'
												> ${p+1} </a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
				...
				<c:choose>
					<c:when test="${!empty annonces }">
						<c:forEach items="${annonces }" var="annonce">
							...
							>
									<div class="row">
										<div class="user col-sm-3 "><img class="photo"
											src="https://d1ovtcjitiy70m.cloudfront.net/vi-1/images/avatar/driver-male-72.png"
											width="72" height="72"
										>
											<div class="user-info">
												<h2 class="username">${annonce.user.firstName }</h2> <br />
										</div></div>
										<div class="description span5 col-sm-5 ">
											...
											<h3 class="fromto">
												...
												<span class="arrow-ie">→</span> <span
													class="trip-roads-stop"
												>${annonce.adresseArrivee }</span> <i aria-hidden="true"
													class="glyphicon glyphicon-road blue size18 tip "
													title="Ce trajet emprunte l'autoroute"
												></i>
											</h3>
											<dl class="geo-from">
												<dt>
													<i class="glyphicon glyphicon-map-marker size18 green"
														title="Départ"
													></i>
												</dt>
											...
											<dl class="geo-to">
												<dt>
													<i class="glyphicon glyphicon-map-marker size18 red"
														title="Arrivée"
													></i>
												</dt>
											...
											<dl class="car-type" title="Confort : normal">
												...
												<dd>
													<span class="rating-car star_2 tip"
														title="Confort : normal"
													></span>
												</dd>
											</dl>
									</div>
										<div class="offer span2 align-right col-sm-4">
											<div class="price price-green"><strong> <span>
														${annonce.prix } € </span>
											</strong> <span class="priceUnit">par place</span></div>
											<div class="availability"><c:if
													test="${annonce.placeDisponible=='1' }"
												>
													<strong>1</strong> place restante
										</c:if> <c:if test="${annonce.placeDisponible=='0' }">
				...
				<nav>
									<ul class="pagination">
										<li><c:if test="${current=='0' }">
												...
												<a class="btn btn-default" aria-label="Previous"
													href='<spring:url value="/chercherAnnonce?adresseD=${adresseD }&adresseA=${adresseA }&page=${current - 1 }"></spring:url>'
												><span aria-hidden="true">&laquo;</span></a>
											</c:if></li>
										<li><c:if test="${!empty annonces }">
												<c:forEach begin="0" end="${maxPages-1 }" var="p">
